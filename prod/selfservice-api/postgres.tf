...
terraform {
  backend "s3" {
    ...
    encrypt        = true
    key            = "selfservice-api-prod/terraform.tfstate" # This is the path to the state file inside the bucket. You can change it to whatever you want.
    region         = "eu-west-1"
...
module "db_instance" {
  ...
  #     Valid Values: .
  #     Notes: .
  identifier = "selfservice-api-prod"
  ...
  source_snapshot_identifier = "selfservice-api-postgres-prod-blueprint-encrypted"
}
